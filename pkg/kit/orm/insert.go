...
func TestInsert(t *testing.T) {
	// INSERT INTO ab (a, b) VALUES (?, ?)
	err := NewDB(nil).INSERT().INTO("ab").COLUMNS("a", "b").VALUES(1, 2).Print(context.Background())
	if err != nil {
...
func TestInsertModel(t *testing.T) {
	// INSERT INTO ab (a, b) VALUES (?, ?)
	err := NewDB(nil).Debug().INSERT(&TestModel{}).INTO("xxx").Print(context.Background())
	if err != nil {
		slog.Error("insert:", "err", err)
	}
}
...
func TestInsertModels(t *testing.T) {
	// INSERT INTO ab (a, b) VALUES (?, ?), (?, ?)
	err := NewDB(nil).Debug().INSERT(&TestModel{}, &TestModel{}).INTO("xxx").Print(context.Background())
	if err != nil {
		slog.Error("insert:", "err", err)
	}
}
