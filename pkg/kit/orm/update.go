...
func TestUpdate(t *testing.T) {
	...
	_, err := NewDB(nil).UPDATE("xx").SET(map[string]any{"a": 1, "b": 2}).WHERE(map[string]any{"a": 1}).Exec(context.Background())
	if err != nil {
...
func TestUpdate1(t *testing.T) {
	// UPDATE xx SET a = 1, b = 2 WHERE a > c
	_, err := NewDB(nil).UPDATE("xx").SET1(&TestModel{}).WHERE(map[string]any{"a": 1}).Exec(context.Background())
	if err != nil {
		slog.Error("update:", "err", err)
	}
}
